...
def test_activate_card_inactive_returns_false():
	pa = ProcessAction()
	card = Card()
	# place 1 pollution into center to deactivate the card
...
def test_activate_card_rejects_when_pollution_cannot_be_placed():
	...
	acting = Card()
	# target card has 0 safe slots, trying to place 2 would require 2 center cubes -> rejected
	...
	assert result is False
	# ensure no pollution was placed
	assert target.pollution_in_center == 0
...
def test_activate_card_rejects_when_outputs_cannot_be_put():
	pa = ProcessAction()
	acting = Card()
	# make target inactive so it cannot accept resources
	target = Card()
	target.place_pollution(1)
...
def test_activate_card_success_performs_actions():
	...
	# produces 1 center pollution and fills safe slots as available
	acting = Card(pollutionSpacesL=2, upperEffect=ArbitraryBasic(from_=2, to=[Resource.MONEY], pollution=3))
	...
	pollution = [pos_target, pos_target, pos_target]

	result = pa.activateCard(acting, grid, inputs=inputs, outputs=outputs, pollution=pollution)

	assert result is True

	# verify pollution placed on the target: 2 safe slots filled and 1 in center
	assert target.pollution_on_slots == 2
	assert target.pollution_in_center == 1
